...
The design follows a **fully expression-based interpreter** approach without imposed limitations. All expressions can be combined with one another, provided no type errors occur. This flexible design ensures that the interpreter maintains the dynamic and versatile nature of the Scheme language.

A key design decision was to leverage the `begin` expression for implementing function code. This provides a consistent and intuitive structure for handling blocks of code in the interpreter, aligning with the principles of functional programming.
