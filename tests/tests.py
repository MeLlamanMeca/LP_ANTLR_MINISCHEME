...
def run_tests(silent=False):
    ...
    test_files = [f for f in os.listdir(input_dir) if f.endswith('.inp')]
    print("----------------------------------------")
    def run_single_test(input_file, output_file, test_name):
        with open(input_file, 'r') as inp, open(output_file, 'r') as out:
    ...
    # Additional test for /iotest/iotest.inp
    iotest_input_file = os.path.join(script_dir, 'iotest/iotest.inp')
